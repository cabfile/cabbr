...
if(isMainThread) { // Safeguard against running this file instead of cabbr.js
	...
	const {stereoTest, sampleRate, expr, mode} = workerData;
	const fakeWindow = {};
	...
	if(!stereoTest) {
		...
		const report = fancy !== 0 && reportEvery > 0 ? t=>{if(t % divisor == stereo) parentPort.postMessage(wnum+';'+((t-range[0])/idk)+'%');} : ()=>{};
		...
		parentPort.postMessage([Array.isArray(testRun),error]);
	}
}
